...
int main (void)
{
	...
	float temperature_matrix[tot_days][tot_hours];
	...
	for (day=0; day<tot_days; day++){
		for (hour=0; hour<tot_hours; hour++){
			temperature_matrix[day][hour] = (rand() %50 + 200) * 0.1;
		}
	}
	...
	for (day=0; day<tot_days; day++){
		...
		for (hour=0; hour<tot_hours; hour++) printf("|%4.1f", temperature_matrix[day][hour]);
	...
	for (day=0; day<tot_days; day++){
		for (hour=0; hour<tot_hours; hour++) sum += temperature_matrix[day][hour]; //comment
	}
